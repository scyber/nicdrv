...
usage()
{
	...
	echo "usage: # $0 LocalName RemoteName driver-name local_interface device-id \ 
	    run_times time_boundary"
	echo " # $0 11.0.1.9 11.0.1.18 e1000g 0 '"pci8086,1010"' 10 5"
}
...
load_unload()
{
	...
	while [ $iCnt -lt $MAX_UNLOAD_TIMES ]; do
		...
		echo "echo ::memstat | mdb -k"
		echo "::memstat" | mdb -k
		kernel_mem_0=$(echo "::memstat" | mdb -k | grep -w Kernel | awk '{print $3}')
		...
		if [ $? -eq 0 ]; then
			...
			echo "::memstat" | mdb -k
			kernel_mem_1=$(echo "::memstat" | mdb -k | grep -w Kernel | awk '{print $3}')
			if (( kernel_mem_1 - kernel_mem_0 > max_mem_size )); then
				echo "WARNING: Kernel memory increased too much adding device driver."
			fi
		else
		...
		# MAXQ now for load_unload can run on server and client
		echo "Run MAXQ benchmark over the link"
		for init_data_len in 65000 64; do
			echo "MAXQ payload: $init_data_len starts at `date`"
		...
		# Repeatedly plumb/unplumb interface with ping traffic
                ${STF_SUITE}/${STF_EXEC}/plumb_unplumb \
                        $g_drv $g_Local_Interface $g_LocalHost \
                        $if_netmask $g_RmHost $g_bounary&
                plu_id=`echo $!`
                echo "plumb_unplumb daemon $plu_id starts at `date`"

		# ping traffic with large size packets for 180 seconds, luo
		ping -s $g_RmHost 65000 180 > /dev/null 2>&1

                kill $plu_id > /dev/null 2>&1
                wait $plu_id
		if [ $? -ne 0 ]; then
			FAIL_FLAG=1
			echo "plumb unplumb use too much time"
		else
			echo "plumb unplumb time check pass!"
		fi
                echo "plumb_unplumb killed at `date`"
		if [ $iCnt -eq 0 ]; then
